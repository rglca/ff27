...
1. **Independent replacement level** — the engine selects 10 QBs, 20 RBs, 20 WRs, 10 TEs, 10 Ks, and 10 DEFs by independent projection, then allocates the 20 league FLEX slots to the best remaining RB/WR/TE options. K and DEF are not FLEX-eligible. The best unselected independent player at each position becomes that position's replacement baseline. If a position does not have enough sourced or estimated projections for its starters plus one replacement, its baseline is left missing; the engine does not substitute an ADP-derived fallback.
...
5. **Provenance-aware weighting** — sourced projections receive the full projection weight; manually supplied projections receive half weight; ADP-derived projections receive zero projection weight whenever ADP is present to prevent circular weighting. The ADP tab's refreshed observations are marked `market`; supplemental estimates are marked `estimated`; only supplied `proj` values default to `estimated`, while projections generated by `fallbackProjection` default to `adp-derived`.
6. **Confidence matrix** — `HIGH` requires a sourced projection, market ADP, two-season history, and an available replacement baseline. `MEDIUM` requires at least two independent signals with a missing or reduced-quality input. `LOW` is used for one or fewer independent signals, ADP-derived-only support, or a missing baseline. The UI cannot show `CONFIDENCE HIGH` alongside `PROJ MISSING` or `PROJ ADP-DERIVED`.
7. **Roster pressure** — boosts RB/WR/TE while required slots are still empty, boosts FLEX-eligible players while FLEX spots are open, adds pressure for the six bench spots after the ten starters are filled, and suppresses an additional QB after the roster slot is filled. K and DEF retain explicit late-round timing suppression: they are suppressed before pick 105, released at pick 105, and suppressed after that unit is filled.
8. **Strategy and health** — adds the small early WR-first/RB-first lens and applies moderate `Q` or severe `O`/manual-injury discounts. There are no QB-specific production weights or pick-110/140/160 cliffs.

Players carry `adpQuality`, `projectionQuality`, and `recentQuality` metadata. The recommendation tags expose those fields as `ADP MARKET`, `ADP EST`, `PROJ SOURCED`, `PROJ ADP-DERIVED`, `HISTORY 1Y`, and confidence levels. The replacement output also exposes each position's replacement player ID and projection quality for verification/debugging.

The shipped-board circularity invariance test passes: changing every ADP-derived fallback projection does not change independent replacement baselines, VOR scales, replacement IDs, or an independently projected player's score. This is a test-backed property of the current scoring implementation and snapshot, not a claim that external source data is permanently current.
